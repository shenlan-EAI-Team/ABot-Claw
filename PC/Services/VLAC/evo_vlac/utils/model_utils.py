...
class GAC_model():
    ...
    def web_trajectory_done(self, task_description, main_image_path, reference_image_path=None, rich=False):
        done_list=self.get_trajectory_done(
            ...
            threshold=0,
            skip=1,
            goal_image=reference_image_path,
        )
    ...
    def get_trajectory_done(self,task:str,image_list:List[Image.Image],ref_image_list:List[Image.Image]=None,batch_num:int=20,rich=False,ref_num=9,threshold=0,skip=1,goal_image:Image.Image=None,apply_threshold=True):
        """
        ...
        if apply_threshold:
            ...
            done_list = [float(one) for one in done_list]
        return done_list
